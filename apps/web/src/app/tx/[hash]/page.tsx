...
function shareCopy(v: TxView, hash: string): { title: string; description: string; image?: string } {
  const a = v.action;
  const state =
    v.status === "mempool" ? "unconfirmed" : `${v.confirmations} confirmation${v.confirmations === 1 ? "" : "s"}`;
  ...
  if (a?.kind === "dispenser" || a?.kind === "refill") {
    ...
    if (d) {
      ...
      return open
        ...
        : {
            title: `${d.asset} dispenser — closed`,
            description: `This dispenser has ended · ${d.dispense_count} sales. See other ways to get ${d.asset} on xcp.io.`,
            image: art(d.asset),
          };
    }
  }
  if (a?.kind === "fairminter") {
    ...
    return {
      title: `${open ? "Mint" : "Fair mint over:"} ${f.asset_longname || f.asset}${open ? " — open now" : ""}`,
      description: open
        ? `Anyone can mint. Terms + progress on xcp.io.`
        : `Minting has closed — see the asset on xcp.io.`,
      image: art(f.asset),
    };
  }
  if (a?.kind === "order") {
  ...
  if (a?.kind === "issuance") {
    const i = a.issuance;
    return {
      title: `${i.asset_longname || i.asset} — issuance`,
      description: `Birth record on xcp.io · ${state}.`,
      image: art(i.asset),
    };
  }
  const kind = a ? KIND_TITLE[a.kind] : null;
  return {
    title: `${kind ? `${kind} — ` : ""}Transaction ${short(hash)}`,
    description: `Counterparty transaction ${hash} (${state}).`,
  };
}
...
export async function generateMetadata({ params }: { params: Promise<{ hash: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${s.title} | XCP.io`,
      description: s.description,
      ...(s.image ? { images: [{ url: s.image }] } : {}),
    },
    twitter: {

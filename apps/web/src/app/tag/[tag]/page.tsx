...
export async function generateMetadata({ params }: { params: Promise<{ tag: string }> }): Promise<Metadata> {
  ...
  const members = detail.entity_type === "asset" ? `${commas(detail.n_assets)} assets` : `${commas(detail.n_addresses)} addresses`;
      ...
      {collection ? (
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
            ...
            <Stat label="Median Rating" value={collection.median_rating == null ? "—" : `${collection.median_rating.toFixed(1)} / 10`} />
            ...
            <Stat label="Holder overlap" value={collection.holder_overlap_pct == null ? "—" : `${collection.holder_overlap_pct}%`} />
            <Stat label="Top member value" value={collection.top_asset_value_pct == null ? "—" : `${collection.top_asset_value_pct}%`} />
          ...
          <div className="rounded-lg border border-[var(--border2)] bg-[var(--surface)] p-5">
            ...
            <div className="mt-3 flex h-3 overflow-hidden rounded-full bg-zinc-900" aria-label="Rated member distribution">
              {[
                ...
                Number(count) > 0 ? (
                  <div key={index} className={String(color)} style={{ width: `${(100 * Number(count)) / collection.rated_members}%` }} />
                ) : null,
              )}
            </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <Stat label="Median Rating" value={detail.median_rating == null ? "—" : `${detail.median_rating.toFixed(1)} / 10`} />

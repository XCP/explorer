import { Stamp, Coins, Layers, Vault, Waves, Clock, Users, AlertTriangle, GitBranch, Hash, Landmark, History, type LucideIcon } from "lucide-react";
...
const CLASS: Record<string, { label: string; Icon: LucideIcon; tone: Tone; title: string }> = {
  ...
  "pre-ethereum":    { label: "Pre-Ethereum",    Icon: Landmark, tone: "provenance", title: "Issued before Ethereum's genesis block (July 30, 2015) — older than the entire Ethereum chain" },
  "pre-cryptopunks": { label: "Pre-CryptoPunks", Icon: History,  tone: "provenance", title: "Issued before CryptoPunks V1 (June 9, 2017) — predates the first Ethereum NFTs" },
  // protocol — what this asset IS
  stamp:    { label: "Stamp",    Icon: Stamp,  tone: "protocol", title: "Bitcoin Stamp — art stored permanently on-chain in the UTXO set" },
  src20:    { label: "SRC-20",   Icon: Coins,  tone: "protocol", title: "SRC-20 — the Bitcoin Stamps fungible-token standard" },
  src721:   { label: "SRC-721",  Icon: Layers, tone: "protocol", title: "SRC-721 — the Bitcoin Stamps composable-NFT standard" },
  // structure — how it was issued (named is the default, so it's left implicit)
  subasset: { label: "Subasset", Icon: GitBranch, tone: "structure", title: "A subasset — a child of a named parent asset" },
  numeric:  { label: "Numeric",  Icon: Hash,   tone: "structure", title: "A numeric asset (A-prefixed), issued without a claimed name" },
  // quality — behavioral signals that feed the rating
  vaulted:  { label: "Vaulted",  Icon: Vault,  tone: "quality", title: "Some supply is wrapped in an Emblem Vault (tradable on Ethereum)" },
  liquid:   { label: "Liquid",   Icon: Waves,  tone: "quality", title: "10+ trades — an active market" },
  durable:  { label: "Durable",  Icon: Clock,  tone: "quality", title: "Traded across a long span — it has staying power" },
  broad:    { label: "Broad",    Icon: Users,  tone: "quality", title: "50+ holders — widely distributed" },
  // integrity
  wash:     { label: "Wash",     Icon: AlertTriangle, tone: "warn", title: "Flagged: wash-trading / low-quality signals — capped to a Speculative rating" },
};
// stable display order regardless of the tag array's order
const ORDER = ["pre-ethereum", "pre-cryptopunks", "stamp", "src20", "src721", "subasset", "numeric", "vaulted", "liquid", "durable", "broad", "wash"];

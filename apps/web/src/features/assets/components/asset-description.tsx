...
const isJsonPointer = (d: string) =>
  /\.json(\?|#|$)/i.test(d) || d.toLowerCase().includes(".json") || /^[@*]https?:\/\//i.test(d);
...
export function AssetDescription({ asset, description }: { asset: string; description: string | null }) {
  ...
  const binaryKind = /^stamp:/i.test(description.trim())
    ? "On-chain STAMP image"
    : /^ord:/i.test(description.trim())
      ? "Ordinals inscription"
      : /^data:/i.test(description.trim())
        ? "Inline data URI"
        : null;
    ...
    <div className="card factcard">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {json && state === "idle" && (
          <button
            onClick={load}
            className="cursor-pointer rounded-md bg-sky-500/10 px-2.5 py-1 text-xs font-medium text-sky-300 ring-1 ring-inset ring-sky-500/20 hover:bg-sky-500/20"
          >
            Inspect JSON
          </button>
        )}
      ...
      <div className="body">
          ...
          <p className="whitespace-pre-wrap [overflow-wrap:anywhere] text-sm leading-relaxed text-zinc-300">
            ...
            {long && (
              <button
                ...
                className="ml-2 cursor-pointer whitespace-nowrap text-xs text-sky-400 hover:text-sky-300"
              >
                {expanded ? "less" : "more"}
              </button>
            )}
        ...
        {state === "error" && (
          <p className="mt-2 text-xs text-red-400">{result?.error || "Could not load the JSON."}</p>
        )}
        {state === "done" && result?.json && (
          <AssetEnhancedInfo
            data={result.json}

...
export function AssetActivityChart({ data }: { data: AssetActivityMonth[] }) {
  ...
  const totals = data.reduce(
    (a, d) => ({
      sends: a.sends + d.sends,
      orders: a.orders + d.orders,
      dispensers: a.dispensers + d.dispensers,
      supply: a.supply + d.supply,
    }),
    { sends: 0, orders: 0, dispensers: 0, supply: 0 },
  );
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-3 py-2 border-b border-[var(--border2)]">
        ...
        {LAYERS.filter((k) => totals[k] > 0).map((k) => (
          <button
            key={k}
            onClick={() => toggle(k)}
            className={`flex items-center gap-1 cursor-pointer ${visible[k] ? "" : "opacity-30"}`}
          >

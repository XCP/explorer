  ...
  return (
    <div className="space-y-4">
      {act == null ? (
        ...
        <div className="card">
          <div className="px-3 py-8 text-center text-xs text-zinc-600">Not enough activity yet to chart.</div>
        </div>
      )}
        ...
        <div className="card factcard">
          ...
          <div className="body">
            <p className="pb-1 text-xs leading-relaxed text-zinc-500">
              Ranked by lifetime <span className="text-zinc-300">credits + debits</span> — who has <em>used</em> {asset}{" "}
              the most (moved it in and out), regardless of what they hold now.
            </p>
            {users.map((u, i) => (
              <div key={u.address} className="row">
                <span className="k inline-flex items-center gap-2">
                  ...
                  <Link href={`/address/${u.address}`} className="font-mono">
                    {short(u.address)}
                  </Link>
                </span>
                <span className="amt mono">
                  {commas(u.activity)}{" "}
                  <span className="time">
                    {commas(u.credits)} in · {commas(u.debits)} out
                  </span>
                </span>
              </div>
            ))}

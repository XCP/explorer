...
export function AddressConnections({ address }: { address: string }) {
  const { data, isLoading } = useSWR<Envelope<AddressConnectionRow[]>>(
    apiUrl(`/v2/addresses/${encodeURIComponent(address)}/connections`),
  );
        ...
        <ul className="space-y-1">
          {rows.map((r) => (
            <li key={r.cp} className="relative rounded overflow-hidden">
              <div
                className="absolute inset-y-0 left-0 bg-(--color-accent)/10"
                style={{ width: `${Math.max(3, (Number(r.interactions) / max) * 100)}%` }}
              />
...
export function AddressLineage({ address }: { address: string }) {
  const { data } = useSWR<Envelope<AddressLineageRow[]>>(
    apiUrl(`/v2/addresses/${encodeURIComponent(address)}/lineage`),
  );
  const rows = data?.result ?? [];
    ...
    <div className="rounded-lg border border-amber-500/20 bg-amber-500/[0.04] px-4 py-2.5 text-sm flex items-start gap-2.5">
      ...
      <div className="flex flex-col gap-0.5">
        <span className="text-zinc-200 font-medium">
          Identity lineage <span className="text-zinc-400 font-normal">· linked by sweep (same owner)</span>
        </span>
        {rows.map((r, i) => (

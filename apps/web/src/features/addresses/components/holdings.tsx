...
export function Holdings({ address }: { address: string }) {
  ...
  const rows = (data?.result ?? [])
    .slice()
    .sort((a, b) => Number(b.quantity_normalized) - Number(a.quantity_normalized));
  const tab = (k: typeof view, label: string) => (
    <button
      onClick={() => setView(k)}
      className={`text-xs px-2 py-0.5 rounded transition-colors ${view === k ? "bg-zinc-800 text-zinc-100" : "text-zinc-400 hover:text-zinc-200"}`}
    >
      {label}
    </button>
  );
            ...
            {rows.map((b) => (
              ...
              >
                <AssetArt
                  asset={b.asset}
                  stamp={!!b.stamp}
                  className="w-full aspect-[3/4] group-hover:scale-105 transition-transform"
                />

  ...
  return (
    <section className="card txdecl">
      {b.text ? (
        ...
        <blockquote className="dim">(empty broadcast)</blockquote>
      ) : null}
      {oracle && (
        <div className="oracle" style={{ marginTop: b.text ? 12 : 0 }}>
          {commas(b.value)}
        </div>
      )}
      <div className="meta">
        — {addr(b.source)}
        {b.timestamp != null && <> · feed timestamp {commas(b.timestamp)}</>}
        {oracle && <> · oracle value</>}
        {!oracle && <> · no value attached</>}
        {b.mime_type && <> · {b.mime_type}</>}
        {b.locked ? <> · feed LOCKED — it can never broadcast again</> : null}
      </div>
    </section>
  );
}
...
export function IssuanceBirth({ action }: { action: Extract<TxAction, { kind: "issuance" }> }) {
  ...
  const sentence = isTransfer ? (
    ...
    <>
      <b>{name}</b> {events.includes("creation") ? "was born" : "grew"} — <b>{commas(i.quantity_normalized)}</b>{" "}
      {i.divisible ? "divisible units" : Number(i.quantity_normalized) === 1 ? "indivisible unit" : "indivisible units"}
      , issued by <b>{addr(i.source)}</b>.
    </>
  ) : (
    <>
      <b>{name}</b> was updated by <b>{addr(i.source)}</b>
  ...
  return (
    <section className="card">
      <div className="txbirth">
        {i.asset && (
          <div className="art">
            <AssetArt asset={i.asset} className="w-full aspect-[5/7] rounded-lg border border-zinc-800" />
          </div>
        )}
        <div>
          ...
          {i.description && (
            <div className="go dim" style={{ wordBreak: "break-all" }}>
              “{i.description}”
            </div>
          )}
          {i.issuer && i.issuer !== i.source && !isTransfer && <div className="go dim">issuer {addr(i.issuer)}</div>}
          <div className="go">
            This page is the birth record — the living page is{" "}
            <Link href={`/asset/${i.asset}`} className="!text-(--color-accent)">
              the {name} asset page
            </Link>
            , with market, holders, and provenance.
          </div>
        </div>
      </div>
    </section>
...
export function BetDecl({ action }: { action: Extract<TxAction, { kind: "bet" }> }) {
  ...
  return (
    <section className="card txdecl">
      <blockquote>
        Wagered <b className="font-mono">{xcpAmt(b.wager_quantity)}</b> that the feed resolves{" "}
        {betTypeBadge(b.bet_type)}
        {b.target_value != null && (
          <>
            {" "}
            at <b className="font-mono">{commas(b.target_value)}</b>
          </>
        )}{" "}
        — against a counterwager of <b className="font-mono">{xcpAmt(b.counterwager_quantity)}</b>.
      </blockquote>
      <div className="meta">
        — {addr(b.source)} · feed {addr(b.feed_address)} · deadline {commas(b.deadline)}
        {b.leverage != null && (
          <>
            {" "}
            · leverage {commas(b.leverage)} <span title="5040 = 1x">/5040</span>
          </>
        )}
        {b.status && <> · {b.status}</>}
      </div>
...
export function RpsDecl({ action }: { action: Extract<TxAction, { kind: "rps" }> }) {
  ...
  return (
    <section className="card txdecl">
      <blockquote>
        Opened a rock-paper-scissors game — <b className="font-mono">{xcpAmt(r.wager)}</b> on the line,{" "}
        {commas(r.possible_moves)} possible moves.
      </blockquote>
      <div className="meta">
        — {addr(r.source)}
        {r.status && <> · {r.status}</>}

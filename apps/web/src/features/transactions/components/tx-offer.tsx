      ...
      {collection && (
        <div className="caption" style={{ marginTop: 2, fontSize: 11, color: "var(--t4)" }}>
          <Link
            ...
            className="!text-zinc-400 hover:!text-(--color-accent)"
          >
            {collectionLabel(collection.tag)}
          </Link>
          {collection.series != null && collection.card != null && (
            <>
              {" "}
              · S{collection.series} C{collection.card}
            </>
          )}
        </div>
      )}
...
export function DispenserStorefront({
  dispenser: d,
  sales,
  totals,
  collection,
      ...
      <>
        {refillNote && (
          <div className="tx-dead">
            <span className="pill cancelled">restock</span>
            <span className="msg">
              This transaction restocked the dispenser below (since {Number(d.status) === 0 ? "sold out" : "closed"}).
            </span>
          </div>
        )}
        <div className="tx-dead">
          ...
          <span className="msg">
            This dispenser {soldOut ? "sold out" : "closed"}
            {d.closed_block_index ? (
              <>
                {" "}
                at block <b>{commas(d.closed_block_index)}</b>
              </>
            ) : null}
            {totals && totals.n > 0 && (
              <>
          ...
          {d.asset && (
            <Link href={`/asset/${d.asset}`} className="!text-(--color-accent)">
              Other ways to get {d.asset} →
            </Link>
          )}
        </div>
  ...
  return (
    <>
      {refillNote && (
        <div className="tx-dead">
          <span className="pill open">restock</span>
          <span className="msg">This transaction restocked the dispenser below.</span>
        </div>
      )}
      <div className="card txstore" style={{ padding: 0 }}>
        ...
        <div className="shop">
          <div className="forsale">
            For sale · dispenser <span className="pill open">open</span>
          </div>
          ...
          <div className="meta">
            <span>
              ...
              {Number(d.dispense_count) === 1 ? "" : "s"}
            </span>
            <span>
              operator <b>{addr(d.source)}</b>
            </span>
            <span>
              opened <b>block {commas(d.block_index)}</b> · {timeAgo(d.block_time)}
            </span>
          </div>
          <div className="stockbar">
            ...
            <div className="lbl">
              <span>
                {commas(remaining)} of {commas(initial)} remaining
              </span>
          ...
          <div className="howto">
            ...
            <div className="btns">
              ...
              {d.asset && (
                <Link className="txbtn ghost" href={`/asset/${d.asset}`}>
                  View {d.asset} →
                </Link>
              )}
            </div>
...
export function FairminterCampaign({ fairminter: f }: { fairminter: FairminterRow }) {
      ...
      <div className="tx-dead">
        ...
        <span className="msg">
          ...
          {Number(f.paid_quantity) > 0 && (
            <>
              {" "}
              · <b>{xcpAmt(f.paid_quantity)}</b> taken in
            </>
          )}
          .
        </span>
        {f.asset && (
          <Link href={`/asset/${f.asset}`} className="!text-(--color-accent)">
            View {name} →
          </Link>
        )}
      </div>
  ...
  return (
    <div className="card txstore" style={{ gridTemplateColumns: "1fr" }}>
      <div className="shop">
        <div className="forsale">
          Fair mint · anyone can mint <span className="pill open">open</span>
        </div>
        <div className="offerline">
          <b>{name}</b>
          {free ? (
        ...
        <div className="meta" style={{ marginTop: 12 }}>
          ...
          {Number(f.paid_quantity) > 0 && (
            <span>
              taken in <b>{xcpAmt(f.paid_quantity)}</b>
            </span>
          )}
          <span>
            opened by <b>{addr(f.source)}</b>
          </span>
        </div>
        <div className="howto mint">
          <div className="h">How to mint</div>
          <div className="step">
            Send a <b>fairmint</b> naming this minter from any Counterparty wallet
            {free ? (
              " — units are credited free until it closes."
            ) : (
              <>
                {" "}
          ...
          <div className="btns">
            {f.asset && (
              <a className="txbtn buy" href={`https://xcpdex.com/${f.asset}`} target="_blank" rel="noopener noreferrer">
                Mint on xcpdex ↗
              </a>
            )}
            {f.asset && (
              <Link className="txbtn ghost" href={`/asset/${f.asset}`}>
                View {name} →
              </Link>
            )}
          </div>
...
function AcceptOfferButton({ order: o }: { order: OrderRow }) {
  ...
  const accept = async () => {
    ...
    const params = {
      give_asset: o.get_asset,
      give_quantity_normalized: o.get_remaining_normalized,
      get_asset: o.give_asset,
      get_quantity_normalized: o.give_remaining_normalized,
      match_order_tx: o.tx_hash,
    };
    if (!w) {
      setMsg("XCP Wallet not detected — install the extension to fill orders here.");
      return;
    }
    try {
      if (w.composeOrder) {
        await w.composeOrder(params);
        setMsg(null);
      } else if (w.request) {
        await w.request({ method: "xcp_composeOrder", params });
        setMsg(null);
      } else {
        await w.connect?.();
        setMsg("Wallet connected — this wallet version can't compose orders yet.");
      }
    } catch {
      setMsg(null); /* user dismissed the wallet dialog — not an error */
    }
  };
  return (
    <>
      <button type="button" className="txbtn buy" onClick={accept}>
        Fill order
      </button>
...
const Kl = ({ k, children }: { k: string; children: React.ReactNode }) => (
  <div className="font-mono text-[13px] leading-[2.05]">
    <span className="inline-block w-[9ch] text-zinc-500">{k}</span>
    {children}
  </div>
);
...
export function OrderOffer({
  ...
  const quoteUsd =
    v.price != null
      ? v.quote === "XCP" && xcp != null
        ? v.price * xcp
        : v.quote === "BTC" && btc != null
          ? v.price * btc
          : null
      : null;
  ...
  const matchCols = (REGISTRY.order_matches!.cols as Col<OrderMatchRow>[]).map((c) =>
    c.label === "View"
      ? { ...c, cell: (r: OrderMatchRow) => viewCell(r.tx0_hash === o.tx_hash ? r.tx1_hash : r.tx0_hash) }
      : c,
  );
  ...
  return (
    <>
      <div className="card txstore" style={art ? undefined : { gridTemplateColumns: "1fr" }}>
        ...
        <div className="shop">
          <div className="forsale">
            DEX order <span className={`pill ${statePill}`}>{state}</span>
          </div>
          <div className="offerline">
            {v.direction === "buy" ? (
              <>
                ...
                · Paying{" "}
                <b>
                  {commas(o.give_quantity_normalized)} {o.give_asset}
                </b>
              </>
            ) : (
              <>
          ...
          {v.price != null && (
            <div className="price">
              <span className="big">
                {v.price >= 1 ? commas(v.price.toFixed(2)) : v.price.toPrecision(4)} {v.quote}
              </span>
              {quoteUsd != null && (
                <span className="usd">
                  ≈ ${commas(quoteUsd >= 100 ? Math.round(quoteUsd) : quoteUsd.toFixed(2))} per {v.base}
                </span>
              )}
            </div>
          )}
          <div className="stockbar">
            ...
            <div className="lbl">
              ...
              {alive ? (
                <span>
                  {commas(o.give_remaining_normalized)} {o.give_asset} ↔ {commas(o.get_remaining_normalized)}{" "}
                  {o.get_asset} still open
                </span>
              ) : (
                <span>
                  {state === "filled"
                    ? "traded in full"
                    : `${commas(o.give_remaining_normalized)} ${o.give_asset} went untraded`}
                </span>
              )}
            </div>
          </div>
          <div className="mt-3">
            <Kl k="pair">
              <a
                href={`https://xcpdex.com/trade/${v.base}_${v.quote}`}
                target="_blank"
                rel="noopener noreferrer"
            ...
            <Kl k="expires">
              block {commas(o.expire_index)}
              {alive && blocksLeft != null && blocksLeft > 0 && (
                <span className="text-zinc-500"> · {blocksEta(blocksLeft)}</span>
              )}
            </Kl>
          ...
          {alive ? (
            <div className="howto take">
              ...
              <div className="step">
                Offer{" "}
                <b>
                  {commas(o.get_remaining_normalized)} {o.get_asset}
                </b>{" "}
                for{" "}
                <b>
                  {commas(o.give_remaining_normalized)} {o.give_asset}
                </b>{" "}
                — the protocol matches you to this order automatically.
              </div>
              <div className="btns">
                <AcceptOfferButton order={o} />
                {v.base && (
                  <Link className="txbtn ghost" href={`/asset/${v.base}`}>
                    View {v.base} →
                  </Link>
                )}
              </div>
              ...
              <div className="step">
                {state === "filled" ? (
                  <>
                    ...
                    {matches.length > 0 && <> — the match is recorded below</>}.
                  </>
                ) : state === "expired" ? (
                  <>
                    <b>
                    ...
                    went untraded — the order closed at block <b>{commas(o.expire_index)}</b>.
                  </>
                ) : state === "cancelled" ? (
                  <>Withdrawn by its maker before it filled.</>
                ) : (

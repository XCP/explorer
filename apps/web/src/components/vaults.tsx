...
export function Vaults() {
  ...
  const s = d?.summary;
  const Addr = (a: string) => (
    <Link href={`/address/${a}`} className="font-mono flex-1 min-w-0 break-all">
      {a}
    </Link>
  );
    ...
    <>
      <div>
        ...
        <p className="text-sm text-zinc-400 mt-1">
          ...
          <span className="text-zinc-300">Counterparty</span> vaults and their ETH-side sales market. Foreign vaults
          (Namecoin, Ordinals, BTC) hold value on other chains and aren&rsquo;t counted as Counterparty.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Board
          title="Most-sold cards (realized USD)"
          rows={d?.top_sold_assets ?? []}
          render={(r) => (
            <>
              {Asset(r.asset, r.asset_longname)}
              {val(usdCompact(r.usd))}
            </>
          )}
        />
        <Board
          title="Most-vaulted assets"
          rows={d?.top_assets ?? []}
          render={(r) => (
            <>
              {Asset(r.asset, r.asset_longname)}
              {val(`${commas(r.vaults)} vaults`)}
            </>
          )}
        />
        <Board
          title="Sales by type"
          rows={d?.sales_by_class ?? []}
          render={(r) => (
            <>
              <span className="flex-1 min-w-0 truncate text-zinc-300">{CLASS_LABEL[r.sale_class] ?? r.sale_class}</span>
              {val(`${commas(r.sales)} · ${usdCompact(r.usd)}`)}
            </>
          )}
        />
      </div>
      <div className="grid lg:grid-cols-2 gap-6">
        <Board
          title="Top vault funders"
          rows={d?.top_funders ?? []}
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.vaults)} funded`)}
            </>
          )}
        />
        <Board
          title="Top vault crackers"
          rows={d?.top_crackers ?? []}
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.vaults)} cracked`)}
            </>
          )}
        />
      </div>

...
const COLS: Col<AssetIndexRow>[] = [
  { label: "Asset", weight: "primary", priority: 1, w: "minmax(160px,1.4fr)", sortKey: "asset", cell: (r) => assetCell(r.asset, r.asset_longname) },
  { label: "Description", priority: 3, w: "minmax(0,1.6fr)", cell: (r) => <span className="desc">{r.description || "—"}</span> },
  { label: "Supply", numeric: true, cellClass: "qty", priority: 1, w: "130px", sortKey: "supply", cell: (r) => commas(r.supply_normalized) },
  { label: "Issuer", priority: 2, w: "minmax(120px,1fr)", cell: (r) => addrCell(r.issuer) },
  { label: "Created", numeric: true, priority: 2, w: "96px", sortKey: "created", cell: (r) => blockCell(r.last_issuance_block_index) },
...
export function AssetIndex() {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="pagehead"><h1>Assets</h1></div>
      <input
        value={query}
      ...
      <AsyncContent isLoading={isLoading} error={error} empty={rows.length === 0} emptyWhat="assets">
        <RecordTable cols={COLS} rows={rows} label="assets" sort={searching ? undefined : sort} onSort={searching ? undefined : onSort} />

...
export function ActivityChart() {
  ...
  const { data } = useSWR<Envelope<Record<string, { t: number; v: number }[]>>>(
    apiUrl("/v2/metrics", { days: 90, include_hidden: 1 }),
  );
  let series = data?.result?.[s] ?? [];
  if (cum && series.length) {
    let run = 0;
    series = series.map((p) => ({ t: p.t, v: (run += p.v) }));
  }
      ...
      <div className="absolute right-3 top-3 flex flex-wrap justify-end gap-1">
        {SERIES.map(([k, lbl]) => (
          <button
            key={k}
            onClick={() => setS(k)}
            className={`text-xs px-2 py-0.5 rounded transition-colors ${s === k ? "bg-zinc-800 text-zinc-100" : "text-zinc-400 hover:text-zinc-200"}`}
          >
            {lbl}
          </button>
        ))}
        <button
          onClick={() => setCum((v) => !v)}
          title="Cumulative"
          className={`text-xs px-2 py-0.5 rounded transition-colors ${cum ? "bg-(--color-accent) text-zinc-950" : "text-zinc-400 hover:text-zinc-200 border border-zinc-700"}`}
        >
          Σ
        </button>
      </div>
      <div className="text-xs text-zinc-400 mb-2">
        {cum ? "Cumulative" : "Daily"} {label} · last {series.length} days indexed
      </div>

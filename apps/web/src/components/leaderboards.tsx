...
export function Leaderboards() {
  ...
  const { data } = useSWR<Envelope<Record<string, LeaderboardRow[]>>>(
    apiUrl("/v2/leaderboards", showLowQ ? { include_hidden: 1 } : {}),
  );
  const d = data?.result ?? {};
  ...
  const Addr = (value: LeaderboardScalar) => {
    ...
    return (
      <Link href={`/address/${address}`} className="font-mono flex-1 min-w-0 break-all">
        {address}
      </Link>
    );
  };
  ...
  return (
    <>
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">
            Derived across the whole chain — who builds, who collects, what endures. Reputation signals, not just
            balances.
          </p>
        </div>
        <label className="flex items-center gap-1.5 text-xs text-zinc-400 cursor-pointer shrink-0 mt-1">
          <input
            type="checkbox"
            checked={!showLowQ}
            onChange={(e) => setShowLowQ(!e.target.checked)}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Board
          ...
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.survived_assets)} assets that landed`)}
            </>
          )}
        />
        <Board
          title="Top collectors"
          rows={d.top_collectors ?? []}
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.assets_held)} held`)}
            </>
          )}
        />
        <Board
          title="Biggest merchants"
          rows={d.top_merchants ?? []}
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.dispense_btc)} BTC dispensed`)}
            </>
          )}
        />
        <Board
          title="Biggest BTC spenders"
          rows={d.biggest_spenders ?? []}
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.btc_spent)} BTC`)}
            </>
          )}
        />
        <Board
          ...
          render={(r) => (
            <>
              {Addr(r.holder)}
              {val(commas(r.quantity_normalized))}
            </>
          )}
        />
        <Board
        ...
        />
        <Board
          title="Top creator hits"
          rows={d.top_hits ?? []}
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.assets_hits)} hits (50+ holders)`)}
            </>
          )}
        />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Board
          ...
          render={(r) => (
            <>
              {Asset(r.asset, r.asset_longname)}
              {val(`${commas(r.months_traded)} mo traded`)}
            </>
          )}
        />
        <Board
          title="Most dispensed (BTC)"
          rows={d.top_dispensed ?? []}
          render={(r) => (
            <>
              {Asset(r.asset, r.asset_longname)}
              {val(`${commas(r.dispense_btc)} BTC`)}
            </>
          )}
        />
        <Board
          title="Broadest holder base"
          rows={d.broadest_holders ?? []}
          render={(r) => (
            <>
              {Asset(r.asset, r.asset_longname)}
              {val(`breadth ${commas(r.holder_breadth)}`)}
            </>
          )}
        />
        <Board
          title="Most creator-held"
          rows={d.most_creator_held ?? []}
          render={(r) => (
            <>
              {Asset(r.asset, r.asset_longname)}
              {val(`${r.pct_creator_holders}% creators`)}
            </>
          )}
        />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Board
          title="Top stamp creators"
          rows={d.top_stamp_creators ?? []}
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.stamps_created)} stamps`)}
            </>
          )}
        />
        <Board
          title="Top stamp collectors"
          rows={d.top_stamp_collectors ?? []}
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.stamps_collected)} held`)}
            </>
          )}
        />
        <Board
          title="Top SRC-20 deployers"
          rows={d.top_src20_deployers ?? []}
          render={(r) => (
            <>
              {Addr(r.address)}
              {val(`${commas(r.src20_deploys)} deploys`)}
            </>
          )}
        />
        <Board
          title="Most-held stamps"
          rows={d.most_held_stamps ?? []}
          render={(r) => (
            <>
              {Asset(r.asset, r.asset_longname)}
              {val(`${commas(r.holders)} holders`)}

  ...
  return (
    <>
      <div>
        ...
        <p className="text-sm text-zinc-400 mt-1">Counterparty&apos;s origin story — the earliest of each kind of on-chain moment, in order.</p>
      </div>
          ...
          <ol className="text-sm">
            {rows.map((r) => (
              <li key={r.key} className="flex flex-wrap items-center gap-x-3 gap-y-0.5 py-2 border-b border-zinc-900 last:border-0">
                ...
                <Link href={`/block/${r.block}`} className="font-mono text-[10px] text-zinc-500 hover:text-zinc-200 w-16 text-right shrink-0">#{commas(r.block)}</Link>
              </li>
            ))}
          </ol>
        )}

  ...
  return (
    <>
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">Counterparty&apos;s origin story — the earliest of each kind of on-chain moment, in order.</p>
      </div>

      <Card>
        {rows.length === 0 ? <Skeleton rows={12} /> : (
          <ol className="text-sm">
            {rows.map((r) => (
              <li key={r.key} className="flex flex-wrap items-center gap-x-3 gap-y-0.5 py-2 border-b border-zinc-900 last:border-0">
                <time className="font-mono text-xs text-zinc-500 w-[5.5rem] shrink-0">{r.date}</time>
                <span className="text-zinc-200 flex-1 min-w-0">{r.label}</span>
                <span className="text-zinc-300 min-w-0 max-w-[55%]"><Entity type={r.type} ref={r.ref} /></span>
                <Link href={`/block/${r.block}`} className="font-mono text-[10px] text-zinc-600 hover:text-zinc-400 w-16 text-right shrink-0">#{commas(r.block)}</Link>
              </li>
            ))}
          </ol>
        )}

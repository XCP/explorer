...
test("network stats snapshot matches exact source counts and totals", () => {
  ...
  db.exec(`
    ...
    CREATE TABLE fairmints (x INTEGER); CREATE TABLE issuances (fee_paid TEXT, status TEXT);
    ...
    CREATE TABLE network_stats_snapshot (
      ...
      holders INTEGER DEFAULT 0, btc_fees REAL DEFAULT 0, xcp_destroyed REAL DEFAULT 0, updated_at INTEGER DEFAULT 0);
  ...
  assert.equal(row.xcp_destroyed, 1); assert.ok(row.updated_at > 0);
});

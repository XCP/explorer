...
test("compact address signals recompute touched identities and converge to zero", async () => {
  ...
  db.exec(`
    INSERT INTO address_dictionary(address) VALUES('trader'),('peer');
    ...
    INSERT INTO assets(asset_id,type,issuer_id,divisible,locked,first_issuance_block_index)
      SELECT asset_id,'asset',(SELECT address_id FROM address_dictionary WHERE address='trader'),0,0,50
    ...
    INSERT INTO sends(event_index,tx_index,tx_hash,block_index,source_id,destination_id,source_address_id,destination_address_id,asset_id,quantity,msg_index)
      ...
      FROM address_dictionary t,address_dictionary p,asset_dictionary a WHERE t.address='trader' AND p.address='peer' AND a.asset='CARD';
    INSERT INTO balances(address_id,asset_id,quantity,updated_event_index)
      SELECT t.address_id,a.asset_id,'1',1 FROM address_dictionary t,asset_dictionary a WHERE t.address='trader' AND a.asset='CARD';
    INSERT INTO transactions(tx_index,tx_hash,block_index,source_id,fee) SELECT 2,randomblob(32),100,address_id,'1000' FROM address_dictionary WHERE address='trader';
    INSERT INTO order_matches(tx0_index,tx1_index,tx0_hash,tx1_hash,tx0_address_id,tx1_address_id,forward_asset_id,forward_quantity,backward_asset_id,backward_quantity,block_index)
      ...
      WHERE t.address='trader' AND p.address='peer' AND a.asset='CARD' AND x.asset='XCP';
  `);
  await rebuildCoreAddressSignals(d1(db), ["trader", "trader"]);
  const row = () => ({
    ...db
      .prepare(
        `SELECT first_block,last_block,out_peers,assets_held,dex_trades,btc_fees FROM address_signals
    WHERE address_id=(SELECT address_id FROM address_dictionary WHERE address='trader')`,
      )
      .get(),
  });
  ...
  await rebuildCoreAddressSignals(d1(db), ["trader"]);
  assert.deepEqual(row(), {
    first_block: null,
    last_block: 0,
    out_peers: 0,

...
import { parseCmcXcpCsv, parseCmcXcpQuotes } from "#ops/lib/cmc-market-data";
...
test("CMC XCP CSV preserves official OHLCV fields and normalizes dates", () => {
  ...
  assert.deepEqual(parseCmcXcpCsv(raw), [
...
test("CMC XCP CSV rejects duplicate days and malformed OHLC", () => {
  ...
  assert.throws(() => parseCmcXcpCsv(`${header}${row}\n${row}`), /duplicate/);
  assert.throws(() => parseCmcXcpCsv(`${header}17-Jul-26,$1.20,$1.21,$1.10,$1.25,$30,$3`), /OHLC/);
});

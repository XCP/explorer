...
const output = arg("output", ".codex-tmp/import-bitcoin-sidecar.sql");
...
const statements = [`-- Generated from verified compact Bitcoin sidecar projections.`];
const metrics = db
  .prepare(
...
for (const row of balances)
  statements.push(`INSERT INTO btc_sidecar_address_balance(address,balance_sats,utxo_count,first_block,last_block,source,source_version,imported_at)
...
statements.push(
  `INSERT INTO btc_sidecar_state(key,value,updated_at) VALUES('coverage_height',${q(String(metrics.at(-1)?.block_height ?? 0))},unixepoch()),('coverage_hash',${q(hash)},unixepoch()),('source_version',${q(String(sourceVersion))},unixepoch()) ON CONFLICT(key) DO UPDATE SET value=CASE WHEN CAST(excluded.value AS INTEGER)>CAST(btc_sidecar_state.value AS INTEGER) THEN excluded.value ELSE btc_sidecar_state.value END,updated_at=excluded.updated_at;`,
);

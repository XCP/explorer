...
$log = 'C:\BitcoinIndex\bitcoin-index-bootstrap.log'
$from = 298319
$target = 959434
$chunk = 10000
Set-Location $root
while ($from -le $target) {
  ...
  $sql = ".codex-tmp/import-bitcoin-index-$from-$to.sql"
  Add-Content $log "$(Get-Date -Format o) START $from-$to"
  & node apps/api/ops/export-bitcoin-index-sql.mjs "--from=$from" "--to=$to" "--output=$sql" *>> $log

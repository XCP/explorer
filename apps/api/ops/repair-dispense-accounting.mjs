...
function run(label, sql, values = []) {
  const result = executeRemoteD1(numericBindings(sql, values));
...
run("clear-stale-responses", "DELETE FROM cache");
const check = run(
  "verify",
  `SELECT
  ...
  (SELECT COUNT(*) FROM trades t JOIN prices p ON p.currency=t.currency AND p.day=date(t.block_time,'unixepoch')
    WHERE t.currency NOT IN ('USD','USDC') AND ABS(t.usd_value-t.total*p.usd)>0.000001) stale_usd`,
)[0];

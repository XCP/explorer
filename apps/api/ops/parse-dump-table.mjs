...
const arg = (n) => {
  ...
  return h ? (h.includes("=") ? h.split("=")[1] : process.argv[process.argv.indexOf(h) + 1]) : undefined;
};
if (!file || !table) {
  console.error("usage: node parse-dump-table.mjs <dump.sql.gz> <table> [--jsonl out.jsonl]");
  process.exit(1);
}
...
rl.on("line", (line) => {
  if (!inBlock) {
    ...
    if (m) {
      inBlock = true;
      cols = m[1].split(",").map((c) => c.replace(/[`"\s]/g, ""));
    }
    return;
  }
  // a following statement for a different table (or DDL) ends our block
  if (stopRe.test(line) && !insertRe.test(line)) {
    rl.close();
    return;
  }
...
rl.on("close", () => {
  ...
  if (Object.keys(typeHist).length) {
    ...
    for (const [t, n] of Object.entries(typeHist).sort((a, b) => b[1] - a[1]))

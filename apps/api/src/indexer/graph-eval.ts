...
export async function graphEval(env: Env): Promise<Record<string, unknown>> {
  const [aT, aD, rT, rD] = [
    await cut(env, "graph_cut_asset_trust"),
    await cut(env, "graph_cut_asset_distrust"),
    await cut(env, "graph_cut_addr_trust"),
    await cut(env, "graph_cut_addr_distrust"),
  ];
  ...
  const aTiers = await one(
    env,
    `SELECT SUM(CASE WHEN ${aTrust} THEN 1 ELSE 0 END) trusted, SUM(CASE WHEN ${aDist} THEN 1 ELSE 0 END) distrusted, COUNT(*) total FROM asset_signals`,
  );
  const rTiers = await one(
    env,
  ...
  const falseFlag = await one(
    env,
    `SELECT COUNT(*) n FROM asset_signals WHERE ${aDist} AND low_quality=0 AND holders>500 AND trades>500`,
  );
  ...
  const contam = await one(
    env,
    `SELECT SUM(CASE WHEN low_quality=0 AND holders>100 AND trades>10 THEN 1 ELSE 0 END) clean, COUNT(*) distrusted FROM asset_signals WHERE ${aDist}`,
  );
  ...
  return {
    ...
    tiers: {
      assets: {
        trusted: aTiers.trusted,
        distrusted: aTiers.distrusted,
        unscored_pct: pct((aTiers.total ?? 0) - (aTiers.trusted ?? 0) - (aTiers.distrusted ?? 0), aTiers.total ?? 0),
      },
      addresses: {
        trusted: rTiers.trusted,
        distrusted: rTiers.distrusted,

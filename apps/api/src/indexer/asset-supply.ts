...
const SUPPLY_EXPR =
  `CAST((COALESCE((SELECT SUM(CAST(i.quantity AS INTEGER)) FROM issuances i WHERE i.asset=assets.asset AND i.status='valid'),0)
...
async function normalizePass(env: Env, where: string, binds: unknown[]): Promise<void> {
  ...
  const stmts = rows.results.map((r) =>
    env.DB.prepare(`UPDATE assets SET supply_normalized=? WHERE asset=?`).bind(normalize(r.supply, r.divisible === 1), r.asset));
...
export async function crawlAssetSupply(env: Env): Promise<Record<string, unknown>> {
  ...
  if ((await getState(env.DB, "asset_supply_done")) !== "1") {
    ...
    ).bind(cursor, BACKFILL_BATCH).first<{ m: number | null }>();
    if (top?.m == null) { await setState(env.DB, "asset_supply_done", "1"); await recomputeXcp(env); return { phase: "backfill", complete: true }; }
    const hi = top.m;
    await env.DB.prepare(`UPDATE assets SET supply = ${SUPPLY_EXPR} WHERE rowid > ? AND rowid <= ? AND asset!='XCP'`).bind(cursor, hi).run();
  ...
  if (todo.length) {
    ...
    await env.DB.prepare(`UPDATE assets SET supply = ${SUPPLY_EXPR} WHERE asset IN (${ph})`).bind(...todo).run();

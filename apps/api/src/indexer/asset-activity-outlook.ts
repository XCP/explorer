...
WITH normalized AS MATERIALIZED (
  ...
  FROM asset_signals signal WHERE ${ELIGIBLE} AND active_trade_months>0 AND last_trade_time IS NOT NULL
), ranked AS (
...
export const ASSET_ACTIVITY_OUTLOOK_RECONCILE_SQL = `DELETE FROM asset_activity_outlook AS outlook
  WHERE NOT EXISTS (
    SELECT 1 FROM asset_signals signal WHERE signal.asset_id=outlook.asset_id
      AND ${ELIGIBLE} AND signal.active_trade_months>0 AND signal.last_trade_time IS NOT NULL
  )`;

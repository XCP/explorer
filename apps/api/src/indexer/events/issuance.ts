...
const issuance: Handler = ({ ev, p, b, bt }, ctx) => {
  ctx.stmts.push((db) => db.prepare(
    `INSERT OR IGNORE INTO issuances (event_index,tx_hash,block_index,block_time,asset,asset_longname,quantity,quantity_normalized,source,issuer,transfer,divisible,locked,description,fee_paid,status,asset_events,mime_type,reset,callable,call_date,call_price,msg_index,tx_index)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).bind(ev.event_index, p.tx_hash ?? null, b, bt, p.asset ?? null, p.asset_longname ?? null,
         p.quantity != null ? String(p.quantity) : null, p.quantity_normalized ?? normalize(p.quantity, !!p.divisible),
         p.source ?? null, p.issuer ?? null, p.transfer ? 1 : 0, p.divisible ? 1 : 0, p.locked ? 1 : 0,
         cap(p.description), p.fee_paid != null ? String(p.fee_paid) : null, p.status ?? "valid", p.asset_events ?? null,
         p.mime_type ?? null, p.reset ? 1 : 0, p.callable ? 1 : 0, p.call_date ?? null, p.call_price != null ? String(p.call_price) : null,
         p.msg_index ?? null, p.tx_index ?? null));

  if (p.asset && (p.status ?? "valid") === "valid") {
    ctx.supplyDirty.add(p.asset);
    const type = p.asset === "BTC" || p.asset === "XCP" ? "native" : p.asset_longname ? "subasset" : p.asset[0] === "A" ? "numeric" : "asset";
    ctx.stmts.push((db) => db.prepare(
    ...
    if (st) {
      const tag = (t: string) => ctx.stmts.push((db) => db.prepare(
        `INSERT OR IGNORE INTO tags (entity_type,entity_id,tag,source) VALUES ('asset',?,?,'protocol')`).bind(p.asset, t));
      tag("stamp");
      const proto = st.protocol === "SRC-20" ? "src20" : st.protocol === "SRC-721" ? "src721" : st.protocol === "SRC-101" ? "src101" : null;
      if (proto) tag(proto);
    ...
    if (p.reset) ctx.stmts.push((db) => db.prepare(`UPDATE assets SET divisible=? WHERE asset=?`).bind(p.divisible ? 1 : 0, p.asset));
  }
};

export const issuances: Record<string, Handler> = {
  ASSET_ISSUANCE: issuance, ASSET_TRANSFER: issuance, RESET_ISSUANCE: issuance,
};

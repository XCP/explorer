...
const open: Handler = ({ p, b, bt }, ctx) => {
  ctx.stmts.push((db) => db.prepare(
    `INSERT OR REPLACE INTO orders (tx_hash,block_index,block_time,source,give_asset,give_quantity,give_remaining,get_asset,get_quantity,get_remaining,expiration,expire_index,fee_required,fee_required_remaining,fee_provided,fee_provided_remaining,status,closed_block_index)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NULL)`
  ).bind(p.tx_hash, b, bt, p.source ?? null, p.give_asset ?? null, str(p.give_quantity), str(p.give_remaining ?? p.give_quantity),
         p.get_asset ?? null, str(p.get_quantity), str(p.get_remaining ?? p.get_quantity), p.expiration ?? null, p.expire_index ?? null,
         str(p.fee_required), str(p.fee_required_remaining ?? p.fee_required), str(p.fee_provided), str(p.fee_provided_remaining ?? p.fee_provided), p.status ?? "open"));
};
...
const status: Handler = ({ ev, p, b, bt }, ctx) => {
  const st = ev.event === "ORDER_EXPIRATION" ? "expired" : ev.event === "CANCEL_ORDER" ? "cancelled"
    : ev.event === "ORDER_FILLED" ? "filled" : (p.status ?? null);
  // CANCEL_ORDER's tx_hash is the cancel TX; the order it cancels is offer_hash/order_hash.
  const hash = ev.event === "CANCEL_ORDER" ? (p.offer_hash ?? p.order_hash) : (p.tx_hash ?? p.offer_hash ?? p.order_hash);
...
const match: Handler = ({ p, b, bt }, ctx) => {
  ctx.stmts.push((db) => db.prepare(
    `INSERT OR REPLACE INTO order_matches (id,tx0_hash,tx1_hash,tx0_address,tx1_address,forward_asset,forward_quantity,backward_asset,backward_quantity,block_index,block_time,status,match_expire_index,fee_paid,tx0_index,tx1_index,tx0_block_index,tx1_block_index,tx0_expiration,tx1_expiration)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).bind(p.id ?? `${p.tx0_hash}_${p.tx1_hash}`, p.tx0_hash ?? null, p.tx1_hash ?? null, p.tx0_address ?? null, p.tx1_address ?? null,
         p.forward_asset ?? null, str(p.forward_quantity), p.backward_asset ?? null, str(p.backward_quantity), b, bt, p.status ?? "completed",
         p.match_expire_index ?? null, p.fee_paid != null ? String(p.fee_paid) : null, p.tx0_index ?? null, p.tx1_index ?? null,
         p.tx0_block_index ?? null, p.tx1_block_index ?? null, p.tx0_expiration ?? null, p.tx1_expiration ?? null));
};

const matchUpdate: Handler = ({ p }, ctx) => { // carries only {order_match_id, status}
  ctx.stmts.push((db) => db.prepare(`UPDATE order_matches SET status=? WHERE id=?`)
    .bind(p.status ?? "completed", p.order_match_id ?? p.id ?? `${p.tx0_hash}_${p.tx1_hash}`));
};

const matchExpire: Handler = ({ p }, ctx) => { // BTC-pair specific (unpaid BTCPay -> match expires)
  ctx.stmts.push((db) => db.prepare(`UPDATE order_matches SET status='expired' WHERE id=?`)
    .bind(p.order_match_id ?? p.id ?? `${p.tx0_hash}_${p.tx1_hash}`));
};

const btcpay: Handler = ({ ev, p, b, bt }, ctx) => { // completes a BTC order match
  ctx.stmts.push((db) => db.prepare(
  ...
  ).bind(ev.event_index, p.tx_hash ?? null, b, bt, p.source ?? null, p.destination ?? null, p.order_match_id ?? null,
         str(p.btc_amount), p.btc_amount_normalized ?? null, p.status ?? "valid"));
  if (p.order_match_id) ctx.stmts.push((db) => db.prepare(`UPDATE order_matches SET status='completed' WHERE id=?`).bind(p.order_match_id));
};

export const orders: Record<string, Handler> = {
  OPEN_ORDER: open,
  ORDER_UPDATE: status, ORDER_FILLED: status, CANCEL_ORDER: status, ORDER_EXPIRATION: status,
  ORDER_MATCH: match, ORDER_MATCH_UPDATE: matchUpdate, ORDER_MATCH_EXPIRATION: matchExpire,
  BTC_PAY: btcpay,
};

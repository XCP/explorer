...
const openRps: Handler = ({ p, b, bt }, ctx) => {
  ctx.stmts.push((db) => db.prepare(
    `INSERT OR REPLACE INTO rps (tx_hash,block_index,block_time,source,possible_moves,wager,move_random_hash,expiration,expire_index,status)
     VALUES (?,?,?,?,?,?,?,?,?,?)`
  ).bind(p.tx_hash, b, bt, p.source ?? null, p.possible_moves ?? null, str(p.wager), p.move_random_hash ?? null,
         p.expiration ?? null, p.expire_index ?? null, p.status ?? "open"));
};

const rpsUpdate: Handler = ({ ev, p }, ctx) => {
  ctx.stmts.push((db) => db.prepare(`UPDATE rps SET status=? WHERE tx_hash=?`)
  ...
  ctx.stmts.push((db) => db.prepare(
    `INSERT OR REPLACE INTO rps_matches (id,tx0_hash,tx0_address,tx1_hash,tx1_address,possible_moves,wager,block_index,block_time,status)
     VALUES (?,?,?,?,?,?,?,?,?,?)`
         ...
         p.possible_moves ?? null, str(p.wager), b, bt, p.status ?? "pending"));
};

const rpsMatchUpdate: Handler = ({ p }, ctx) => { // carries only {id, status} (the match conclusion)
  ctx.stmts.push((db) => db.prepare(`UPDATE rps_matches SET status=? WHERE id=?`)
    .bind(p.status ?? "pending", p.id ?? p.rps_match_id ?? `${p.tx0_hash}_${p.tx1_hash}`));
};

const rpsMatchExpire: Handler = ({ p }, ctx) => {
  ctx.stmts.push((db) => db.prepare(`UPDATE rps_matches SET status='expired' WHERE id=?`).bind(p.rps_match_id ?? p.id ?? `${p.tx0_hash}_${p.tx1_hash}`));
};

const rpsResolve: Handler = () => { /* see note above — intentionally no-op on rps_matches.status */ };

export const rps: Record<string, Handler> = {
  OPEN_RPS: openRps, RPS_UPDATE: rpsUpdate, RPS_EXPIRATION: rpsUpdate,
  RPS_MATCH: rpsMatch, RPS_MATCH_UPDATE: rpsMatchUpdate, RPS_MATCH_EXPIRATION: rpsMatchExpire,

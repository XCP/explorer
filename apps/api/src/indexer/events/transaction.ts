...
const newTransaction: Handler = ({ p, b, bt }, ctx) => {
  ctx.stmts.push((db) => db.prepare(
    `INSERT OR REPLACE INTO transactions (tx_index,tx_hash,block_index,block_time,source,destination,btc_amount,fee,data,supported,utxos_info)
     VALUES (?,?,?,?,?,?,?,?,?,?,?)`
  ).bind(p.tx_index, p.tx_hash, b, bt, p.source ?? null, p.destination ?? null,
         p.btc_amount != null ? String(p.btc_amount) : null, p.fee != null ? String(p.fee) : null,
         null, p.supported === false ? 0 : 1, p.utxos_info ?? null));
};

...
async function enumEtherscan(key: string, contract: string, fromBlock: number): Promise<{ ids: string[]; cursor: string }> {
  ...
  for (const topic0 of [TRANSFER_SINGLE, TRANSFER_BATCH]) {
    ...
    for (const log of d.result) {
      ...
      else { const n = parseInt(w[2] || "0", 16); for (let i = 0; i < n; i++) ids.add(hexToDec("0x" + (w[3 + i] || "0"))); }
    }
  }
...
export async function crawlEmblemStep(env: Env): Promise<Record<string, unknown>> {
  ...
  if (contracts.length && (ak || ek)) {
    ...
    try {
      ...
      if (ak && !usingEs) {
        ...
        try { r = await enumAlchemy(ak, contract, cursor); out.provider = "alchemy"; }
        catch (e) { if (!ek) throw e; const es = await enumEtherscan(ek, contract, 0); out.provider = "etherscan(fallback)"; r = { rows: es.ids.map((id) => ({ id, btc: null })), cursor: es.cursor ? "es:" + es.cursor : "" }; }
        ...
        for (let i = 0; i < r.rows.length; i += 50) {
          await env.DB.batch(r.rows.slice(i, i + 50).map((x) =>
            env.DB.prepare(`INSERT INTO emblem_vaults (token_id,contract,btc_address,resolved,first_seen) VALUES (?,?,?,?,?)
        ...
        out.enumerated = r.rows.length; nextCursor = r.cursor;
      } else if (ek) {
        const es = await enumEtherscan(ek, contract, parseInt(cursor.slice(3) || "0", 10)); out.provider = "etherscan";
        for (let i = 0; i < es.ids.length; i += 50) {
          await env.DB.batch(es.ids.slice(i, i + 50).map((id) =>
            env.DB.prepare(`INSERT INTO emblem_vaults (token_id,contract,resolved,first_seen) VALUES (?,?,0,?) ON CONFLICT(token_id) DO NOTHING`).bind(id, contract, now)));
        }
  ...
  const rows = (await env.DB.prepare(`SELECT token_id FROM emblem_vaults WHERE resolved=0 LIMIT ?`).bind(RESOLVE_PER_STEP).all<{ token_id: string }>()).results;
  let resolved = 0;
  for (let i = 0; i < rows.length; i += META_CONCURRENCY) {
    ...
    if (done.length) await env.DB.batch(done.map((r) => env.DB.prepare(`UPDATE emblem_vaults SET btc_address=?, resolved=1 WHERE token_id=?`).bind(r.btc, r.id)));

/**
 * Counterparty wire-format codec — the two pure functions for safely decoding CP's JSON.
 *
 * Kept separate from the engine and the message handlers because read surfaces (verify, stats/mempool)
 * also need parseCpJson, and the supply recompute needs normalize.
 */

// CP serializes integer quantity fields (quantity, *_quantity, earned, fee_paid, ...) as BARE JSON

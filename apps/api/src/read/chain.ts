...
async function composeAction(db: D1Database, hash: string, blockIndex: number): Promise<TxAction | null> {
  ...
  if (!kind) return null;
  switch (kind) {
    case "sends": return { kind: "send", sends: await recordsByTxHash(db, "sends", hash, blockIndex) };
    case "dispenses": {
    ...
    case "btcpays": return { kind: "btcpay", btcpay: (await recordsByTxHash(db, "btcpays", hash, blockIndex))[0] };
    case "issuances": return { kind: "issuance", issuance: (await recordsByTxHash(db, "issuances", hash, blockIndex))[0] };
    case "fairminters": return { kind: "fairminter", fairminter: (await recordsByTxHash(db, "fairminters", hash, blockIndex))[0] };
    case "fairmints": {
      const fairmint = (await recordsByTxHash(db, "fairmints", hash, blockIndex))[0];
      const fairminter = fairmint.fairminter_tx_hash ? (await recordsByTxHash(db, "fairminters", fairmint.fairminter_tx_hash))[0] ?? null : null;
      return { kind: "fairmint", fairmint, fairminter };
    }
    case "broadcasts": return { kind: "broadcast", broadcast: (await recordsByTxHash(db, "broadcasts", hash, blockIndex))[0] };
    case "sweeps": return { kind: "sweep", sweep: (await recordsByTxHash(db, "sweeps", hash, blockIndex))[0] };
    case "dividends": return { kind: "dividend", dividend: (await recordsByTxHash(db, "dividends", hash, blockIndex))[0] };
    case "burns": return { kind: "burn", burn: (await recordsByTxHash(db, "burns", hash, blockIndex))[0] };
    case "destructions": return { kind: "destruction", destruction: (await recordsByTxHash(db, "destructions", hash, blockIndex))[0] };
    case "bets": return { kind: "bet", bet: (await recordsByTxHash(db, "bets", hash, blockIndex))[0] };
    case "rps": return { kind: "rps", rps: (await recordsByTxHash(db, "rps", hash, blockIndex))[0] };
    case "pool_liquidity": return { kind: "pool_liquidity", liquidity: (await recordsByTxHash(db, "pool_liquidity", hash, blockIndex))[0] };
    case "pool_matches": return { kind: "pool_swap", swap: (await recordsByTxHash(db, "pool_matches", hash, blockIndex))[0] };
    default: return null;
  }
}
...
function actionValidity(a: TxAction | null): TxView["protocol"] {
  if (!a) return null;
  const status =
    a.kind === "send" ? a.sends[0]?.status
    : a.kind === "order" ? a.order.status
    : a.kind === "cancel" ? a.cancel.status
    : a.kind === "btcpay" ? a.btcpay.status
    : a.kind === "issuance" ? a.issuance.status
    : a.kind === "fairminter" ? a.fairminter.status
    : a.kind === "fairmint" ? a.fairmint.status
    : a.kind === "broadcast" ? a.broadcast.status
    : a.kind === "sweep" ? a.sweep.status
    : a.kind === "dividend" ? a.dividend.status
    : a.kind === "burn" ? a.burn.status
    : a.kind === "destruction" ? a.destruction.status
    : a.kind === "bet" ? a.bet.status
    : a.kind === "rps" ? a.rps.status
    : a.kind === "pool_liquidity" ? a.liquidity.status
    : null;
...
chain.get("/v2/transactions/:hash", async (c) => {
  ...
  if (t) {
    const [tip, action] = await Promise.all([
      blockTip(c.env.DB),
      composeAction(c.env.DB, t.tx_hash, t.block_index).catch((e) => { console.log("tx action compose failed:", e instanceof Error ? e.message : String(e)); return null; }),
    ]);
    const body: TxView = {
      ...t, status: "confirmed",
      confirmations: tip ? Math.max(1, tip - t.block_index + 1) : 1,
      tip, action, pending: [], protocol: actionValidity(action),
    };
  ...
  const body: TxView = {
    tx_hash: hash, status: "mempool", confirmations: 0, tip: null, action: null, pending, protocol: null,
    source: pending[0].source, destination: pending[0].destination,
  };
...
chain.get("/v2/transactions/:hash/events", async (c) => {
  const hash = c.req.param("hash");
  try {
    const r = await fetch(`${c.env.COUNTERPARTY_API_BASE}/transactions/${encodeURIComponent(hash)}/events?verbose=true&limit=100`, { signal: AbortSignal.timeout(8000) });
    if (!r.ok) return J(c, { result: [] as TxEvent[] }, 30);
    const j = parseCounterpartyJson(await r.text()) as { result?: { event?: string; event_index?: number; params?: Record<string, unknown> }[] };
    const events: TxEvent[] = (j.result ?? []).map((e) => ({ event: String(e.event ?? ""), event_index: e.event_index ?? null, params: e.params ?? null }));
...
interface BitcoindVout { value?: number; n?: number; scriptPubKey?: { address?: string; type?: string } }
interface BitcoindVin { coinbase?: string; txid?: string; vout?: number; prevout?: BitcoindVout }
interface BitcoindTx { fee?: number; size?: number; vsize?: number; weight?: number; vin?: BitcoindVin[]; vout?: BitcoindVout[] }
const sats = (btc?: number) => (btc != null && Number.isFinite(btc) ? Math.round(btc * 1e8) : null);

chain.get("/v2/transactions/:hash/bitcoin", async (c) => {
  const hash = c.req.param("hash");
  try {
    const r = await fetch(`${c.env.COUNTERPARTY_API_BASE}/bitcoin/transactions/${encodeURIComponent(hash)}?verbose=true`, { signal: AbortSignal.timeout(8000) });
    ...
    const vin: BitcoinTxIo[] = (t.vin ?? []).map((v) => v.coinbase
    ...
    const body: BitcoinTxSummary = {
      fee_sats: sats(t.fee), size: t.size ?? null, vsize: t.vsize ?? null, weight: t.weight ?? null, vin, vout,
    };

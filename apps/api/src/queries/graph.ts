...
async function tierCounts(db: D1Database, table: "address_signals" | "asset_signals", cuts: GraphCuts): Promise<GraphTierCounts> {
  const r = await one<{ trusted: number; distrusted: number; total: number; zero: number }>(db,
    `SELECT
  ...
  const trusted = r?.trusted ?? 0, distrusted = r?.distrusted ?? 0;
  return {
    trusted,
    distrusted,

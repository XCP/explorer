...
const earliest = (table: string, ref: string, opts: { where?: string; by?: string; bcol?: string } = {}): string => {
...
const viss = (extra: string) =>
  earliest("issuances", "asset ref, 'asset' typ", { where: `status='valid' AND (${extra})`, by: "tx_index" });
...
export const FIRSTS: First[] = [
  // --- protocol genesis ---
  {
    key: "block",
    label: "First block",
    sql: earliest("blocks", "CAST(block_index AS TEXT) ref, 'block' typ", { by: "block_index" }),
  },
  {
    key: "transaction",
    label: "First transaction",
    sql: earliest("transactions", "tx_hash ref, 'tx' typ", { by: "tx_index" }),
  },
  ...
  // --- assets ---
  {
    ...
    sql: earliest("issuances", "asset ref, 'asset' typ", { by: "tx_index" }),
  },
  {
    key: "subasset",
    label: "First subasset",
    sql: earliest("assets", "asset ref, 'asset' typ", { where: "type='subasset'", bcol: "first_issuance_block_index" }),
  },
  {
    key: "numeric",
    label: "First numeric asset",
    sql: earliest("assets", "asset ref, 'asset' typ", { where: "type='numeric'", bcol: "first_issuance_block_index" }),
  },
  {
    key: "destruction",
    label: "First destruction",
    sql: earliest("destructions", "asset ref, 'asset' typ", { by: "event_index" }),
  },
  // --- transfers & markets ---
  ...
  { key: "dispenser", label: "First dispenser", sql: earliest("dispensers", "asset ref, 'asset' typ") },
  {
    ...
    sql: earliest("dispenses", "asset ref, 'asset' typ", { by: "event_index" }),
  },
  // --- other message types ---
  ...
  { key: "btcpay", label: "First BTC pay", sql: earliest("btcpays", "tx_hash ref, 'tx' typ") },
  {
    ...
    sql: earliest("orders", "tx_hash ref, 'tx' typ", { where: "give_asset!='XCP' AND get_asset!='XCP'" }),
  },
  {
    key: "btc_dispense",
    label: "First dispense paid in BTC",
    sql: earliest("dispenses", "asset ref, 'asset' typ", { where: "btc_amount>0", by: "event_index" }),
  },
  ...
  { key: "easyasset", label: "First EasyAsset", sql: viss(`lower(description) LIKE '%easyasset%'`) },
  {
    ...
    sql: `SELECT block_index b, block_time t, asset ref, 'asset' typ FROM fairminters ORDER BY block_index, rowid LIMIT 1`,
  },
  {
    key: "fairmint",
    label: "First fairmint",
    sql: `SELECT block_index b, block_time t, asset ref, 'asset' typ FROM fairmints ORDER BY block_index, rowid LIMIT 1`,
  },
  ...
  // genesis block 779652). The community/protocol Stamp #0 is A7337447728884561000; we display its real block/date.
  {
    ...
    sql: `SELECT block_index b, block_time t, asset ref, 'asset' typ FROM issuances WHERE asset='A7337447728884561000' ORDER BY block_index LIMIT 1`,
  },
  {
    key: "src20",
    label: "First SRC-20 token",
    sql: `SELECT i.block_index b, i.block_time t, i.asset ref, 'asset' typ FROM issuances i JOIN tags tg ON tg.entity_type='asset' AND tg.entity_id=i.asset AND tg.tag='src20' WHERE instr(lower(i.description),'stamp:')>0 ORDER BY i.block_index LIMIT 1`,
  },
  {
    key: "src721",
    label: "First SRC-721 token",
    sql: `SELECT i.block_index b, i.block_time t, i.asset ref, 'asset' typ FROM issuances i JOIN tags tg ON tg.entity_type='asset' AND tg.entity_id=i.asset AND tg.tag='src721' WHERE instr(lower(i.description),'stamp:')>0 ORDER BY i.block_index LIMIT 1`,
  },
  {
    key: "btns",
    label: "First BTNS broadcast",
    sql: `SELECT block_index b, block_time t, source ref, 'address' typ FROM broadcasts WHERE btns=1 ORDER BY block_index, rowid LIMIT 1`,
  },
];

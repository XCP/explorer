 ...
 * Asset Rating comes from its one materialized projection; Conviction remains a config-driven holder profile.
 *
...
const AGG = (convExpr: string, whereTag: boolean) => `
  WITH mem AS (
  ...
  asset_mem AS (
    ...
    LEFT JOIN asset_ratings rating ON rating.asset_id=d.asset_id
  ),
  ranked AS (
    SELECT tag, rating, ROW_NUMBER() OVER (PARTITION BY tag ORDER BY rating) rn,
      COUNT(*) OVER (PARTITION BY tag) cnt FROM asset_mem WHERE rating IS NOT NULL
  ),
  med AS (
    SELECT tag, ROUND(AVG(CASE WHEN rn IN ((cnt+1)/2, (cnt/2)+1) THEN rating END), 1) median_rating
    FROM ranked GROUP BY tag
  ),
  astat AS (
  ...
  SELECT c.tag, c.entity_type, c.source, c.meta, c.n, COALESCE(a.n_assets, 0) n_assets, c.n_addresses,
    a.mean_rating, m.median_rating, a.avg_conviction, a.avg_holder_dex, a.avg_creator_pct, a.pct_low_quality,
...
export function listTagStats(db: D1Database, convExpr: string): Promise<TagStatsBase[]> {
  return q<TagStatsBase>(db, `${AGG(convExpr, false)} ORDER BY c.tag`);
}

/** The aggregate row for a single tag (same math, scoped to one tag). Null when the tag doesn't exist. */
export function getTagStats(db: D1Database, convExpr: string, tag: string): Promise<TagStatsBase | null> {
  return one<TagStatsBase>(db, AGG(convExpr, true), tag);
}
...
export type TagMemberQueryRow = TagMemberRow;
...
export function listTagAssetMembers(
  ...
  return q<TagMemberQueryRow>(
    db,
    `WITH s AS (
       SELECT d.asset, a.asset_longname, signal.*,rating.rating
       FROM tags t

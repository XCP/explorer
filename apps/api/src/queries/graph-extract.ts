...
): { clusterOf: Map<string, number>; stats: Omit<GraphStats, "peripheral"> } {
...
export async function addressEgo(
  db: D1Database,
  addr: string,
  limit: number,
  ...
  const nodes: GraphNode[] = [
    {
      ...
      weight: Math.max(...nbrs.map((n) => n.w), 1),
      center: true,
      cluster: -1,
    },
    ...peripheral.map((id) => ({
      ...
      label: id.slice(0, 8) + "…",
      weight: wById.get(id) ?? 0,
      cluster: clusterOf.get(id) ?? -1,
    })),
  ];
...
export async function assetHolders(
  db: D1Database,
  asset: string,
  limit: number,
): Promise<{ nodes: GraphNode[]; edges: GraphEdge[]; stats: GraphStats }> {
  ...
  const nodes: GraphNode[] = [
    ...
    ...holders.map((h) => ({
      id: h.holder,
      kind: "address" as const,
      label: h.holder.slice(0, 8) + "…",
      weight: h.qty,
      cluster: clusterOf.get(h.holder) ?? -1,
    })),
  ];

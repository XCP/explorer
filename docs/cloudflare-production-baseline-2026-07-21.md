...
- CDN Worker version `dd1ed650-26f7-424d-bf1f-df39522508c2` explicitly stores ordinary image GET responses in the
...
- Missing decorative `logo` and `logo-icon` objects use a bounded negative cache: five minutes in browsers and one hour
  ...
  artwork placeholders use a separate one-minute private browser cache and five-minute shared edge cache. Live probes
  verified canonical MISS to legacy-alias HIT without the zone's one-day Browser TTL override. Transient-error
  placeholders remain `no-store`.
